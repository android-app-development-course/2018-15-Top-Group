...
public class AddEvent extends AppCompatActivity  implements  TimePickerDialog.OnTimeSetListener {
    ...
    private void initView() {
        ...
        action_barview.setRightListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                //--------------------save()中的用户名、组名。
                if (addEventText.getText().toString().isEmpty()) {
                    Toast.makeText(AddEvent.this,"输入不能为空",Toast.LENGTH_SHORT).show();
                    return ;
                }
                if (cal != null) save(username,groupname,one[0],cal.getTimeInMillis(),addEventText.getText().toString());
                Intent intent=new Intent(AddEvent.this,MainActivity.class);
                //intent.putExtra("groupid",BoomSQL.getGroup().getObjectId());
                startActivity(intent);
                finish();
            }
        });
    ...
    public void save(String username,String groupname,int color,Long mill,String content){
        com.example.liyixun.TopGroup.Calendar.Calendar calendar=new Calendar();
